...
def encode(payload):
    ...
    n = 50000
    parts = [payload_as_string[i : i + n] for i in range(0, len(payload_as_string), n)]
            ...
            payloads.append(
                {
                    ...
                    "data": {
                        ...
                        if record.get("dynamodb", {}).get("OldImage") is not None
                        else None,
                    },
                }
            )
...
def _get_handler(handler):
    try:
        ...
        raise Exception(
            "Bad handler '{}': expected format 'module.function'".format(handler)
        ) from e
    m = importlib.import_module(modname.replace("/", "."))
    try:
        return getattr(m, fname)
    except AttributeError:
        raise Exception("Handler '{}' missing on module '{}'".format(fname, modname))
...
def handler(event, context):
    ...
    current_event = event
    original_handler = _get_handler(original_handler_name)
